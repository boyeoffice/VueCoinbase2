<template>
	<div :class="BoxClass">
		<div class="box-header with-border">
            ...
            <router-link v-for="url in links" style="margin-left: 5px;" :class="url.className" :to="url.link" :key="url.id">{{url.label}}</router-link>
...
<script>
	export default{
        ...
        methods: {
            ...
            buildURL() {
                var p = this.params
                return `${this.source}?column=${p.column}&status=${p.status}&direction=${p.direction}&per_page=${p.per_page}&page=${p.page}&status_2=${p.status_2}&search_column=${p.search_column}&search_operator=${p.search_operator}&search_query_1=${p.search_query_1}&search_query_2=${p.search_query_2}`
            }
        }
	}
</script>
